# Instalación limpia AIO de SOC Operations 0.1.150
...
El release `0.1.150` no es compatible con Wazuh 4.12. El `preflight` lo rechaza antes de instalar
...
curl --fail --location --remote-name \
  https://github.com/devsecops-kriptome/soc-operations-installer/releases/download/v0.1.150/soc-operations-0.1.150.tar.gz.age
curl --fail --location --remote-name \
  https://github.com/devsecops-kriptome/soc-operations-installer/releases/download/v0.1.150/SHA256SUMS

grep 'soc-operations-0.1.150.tar.gz.age$' SHA256SUMS | sha256sum --check
```

El resultado debe ser:

```text
soc-operations-0.1.150.tar.gz.age: OK
```
...
age --decrypt \
  ...
  --output soc-operations-0.1.150.tar.gz \
  soc-operations-0.1.150.tar.gz.age

sha256sum --check SHA256SUMS
tar -xzf soc-operations-0.1.150.tar.gz
cd release-0.1.150
sha256sum --check SHA256SUMS
...
Desde `$HOME/soc-installer/release-0.1.150` instale solo el orquestador:
...
Para el release `0.1.150` publicado, ambas huellas deben ser:

```text
b95b569d25fae09448cdcee26f091869c344367bfa79c588d274cc44c7b78323
```
...
sudo /usr/local/sbin/soc-operations-install preflight \
  --staging-root "$HOME/soc-installer/release-0.1.150"
```
...
El instalador `0.1.150` crea los directorios `/opt/soc-operations-lab` y
...
El agente de despliegue valida únicamente su listener `8443`; el listener `9443` pertenece al
gateway Nginx de la API externa y debe existir antes de instalar el agente.

Si una instalación `0.1.148` quedó detenida después de configurar OpenBao con el mensaje
`port 9443 is already owned outside this installer`, descargue y verifique `0.1.150`, instale el
...
`first-engineer`, aplique `0.1.150` con los mismos parámetros y ejecute `resume`. El instalador
