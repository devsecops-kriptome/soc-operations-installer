# Instalación limpia AIO de SOC Operations 0.1.143
...
El release `0.1.143` no es compatible con Wazuh 4.12. El `preflight` lo rechaza antes de instalar
...
curl --fail --location --remote-name \
  https://github.com/devsecops-kriptome/soc-operations-installer/releases/download/v0.1.143/soc-operations-0.1.143.tar.gz.age
curl --fail --location --remote-name \
  https://github.com/devsecops-kriptome/soc-operations-installer/releases/download/v0.1.143/SHA256SUMS

grep 'soc-operations-0.1.143.tar.gz.age$' SHA256SUMS | sha256sum --check
```

El resultado debe ser:

```text
soc-operations-0.1.143.tar.gz.age: OK
```
...
age --decrypt \
  ...
  --output soc-operations-0.1.143.tar.gz \
  soc-operations-0.1.143.tar.gz.age

sha256sum --check SHA256SUMS
tar -xzf soc-operations-0.1.143.tar.gz
cd release-0.1.143
sha256sum --check SHA256SUMS
...
Desde `/root/soc-installer/release-0.1.143` instale solo el orquestador:
...
sudo /usr/local/sbin/soc-operations-install preflight \
  --staging-root /root/soc-installer/release-0.1.143
```
