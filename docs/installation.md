# Instalación limpia AIO de SOC Operations 0.1.151
...
El release `0.1.151` no es compatible con Wazuh 4.12. El `preflight` lo rechaza antes de instalar
...
curl --fail --location --remote-name \
  https://github.com/devsecops-kriptome/soc-operations-installer/releases/download/v0.1.151/soc-operations-0.1.151.tar.gz.age
curl --fail --location --remote-name \
  https://github.com/devsecops-kriptome/soc-operations-installer/releases/download/v0.1.151/SHA256SUMS

grep 'soc-operations-0.1.151.tar.gz.age$' SHA256SUMS | sha256sum --check
```

El resultado debe ser:

```text
soc-operations-0.1.151.tar.gz.age: OK
```
...
age --decrypt \
  ...
  --output soc-operations-0.1.151.tar.gz \
  soc-operations-0.1.151.tar.gz.age

sha256sum --check SHA256SUMS
tar -xzf soc-operations-0.1.151.tar.gz
cd release-0.1.151
sha256sum --check SHA256SUMS
...
Desde `$HOME/soc-installer/release-0.1.151` instale solo el orquestador:
...
Para el release `0.1.151` publicado, ambas huellas deben ser:

```text
2fff351367f9fad3e328604ec8193ee5f6be9e315eee4346bf538c4ff5fb7ab8
```
...
El instalador `0.1.151` crea los directorios `/opt/soc-operations-lab` y
...
Antes de crear el entorno del agente, el instalador resuelve la versión efectiva de
