...
Si una instalación queda interrumpida después de solicitar la contraseña, no repita
`openbao-init`, no borre el estado y no reinstale Wazuh. Instale el orquestador verificado del
release vigente y reconcilie los componentes con:
...
operación anterior se revierte de forma transaccional cuando ese paso falla, por lo que debe
introducir nuevamente la contraseña elegida.
