...
## 1. Alcance y estado en SOC Operations 0.1.148
...
SOC Operations. En `0.1.148` se administra con archivos `root-only` y servicios systemd. No guarde
...
Entre al directorio extraído del release `0.1.148` e instale dependencias. En Ubuntu 24.04:
...
Copie también el release `0.1.148` al Indexer. Suponga que el bundle individual quedó en
`/root/geoip-client`:
...
En `0.1.148` no existe todavía un interruptor en la interfaz web.
...
y controlar estos servicios, pero esa función no forma parte de `0.1.148`.
