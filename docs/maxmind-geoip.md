...
## 1. Alcance y estado en SOC Operations 0.1.146
...
SOC Operations. En `0.1.146` se administra con archivos `root-only` y servicios systemd. No guarde
...
Entre al directorio extraído del release `0.1.146` e instale dependencias. En Ubuntu 24.04:

```bash
cd "$HOME/soc-installer/release-0.1.146"
sudo apt-get update
...
Copie también el release `0.1.146` al Indexer. Suponga que el bundle individual quedó en
`/root/geoip-client`:
...
En `0.1.146` no existe todavía un interruptor en la interfaz web.
...
y controlar estos servicios, pero esa función no forma parte de `0.1.146`.
